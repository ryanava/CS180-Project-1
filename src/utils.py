...
def naive_align(type_of_alignment, input, anchor, search_area):
    ...
    return np.roll(input, shift=best_shift, axis=(0, 1))
    
    

def pre_align_crop(img, margin=0.1):
    #margin is a percentage so i dont magic number it
    y, x = img.shape
    top = int(y * margin)
...
def pyramid_shift(input, anchor_array, smallest_img_size, naive_search_algorithm="ncc"):
    ...
    # recursive call (returns a shift vector) -- the thing i need to print
    smaller_shift = pyramid_shift(input_rescaled, anchor_rescaled, smallest_img_size, naive_search_algorithm="ncc")

    # scale up the shift for the next recurssion layer
...
def automatic_cropping_black(original_image, manipulated_image):
    ### FIRST CROP THE GRAYSCALE (black bars)
    threshold = 4
    edges = 16
    # top check(rows)
    top_candidates = []
...
def automatic_cropping_color(manipulated_image):
    ...
    def check_condition(pixels):
        ...
        #print the full r,g,b or x,y when it hits 3118
